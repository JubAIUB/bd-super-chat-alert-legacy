...
public class MainActivity extends AppCompatActivity implements SmsReceiver.OnDonationReceivedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Fix SSL on old Android (6.x) — updates security provider to support Let's Encrypt
        try {
            ProviderInstaller.installIfNeeded(this);
            Log.d("MainActivity", "Security provider updated successfully");
        } catch (Exception e) {
            Log.e("MainActivity", "Security provider update failed: " + e.getMessage());
        }
